...
Определите переменную окружения `APIKEY`. Для этого необходимо получить [API Яндекс-геокодера](https://dvmn.org/encyclopedia/api-docs/yandex-geocoder-api/). Далее в файл `.env` в каталоге `star_burger/` занести полученный `API ключ`:
```sh
APIKEY=ключ
```
...
## Простое обновление кода на сервере
Имеется возможность обновить все имеющиеся данные, библиотеки, код репозитория и т.д. одной командой:
```sh
